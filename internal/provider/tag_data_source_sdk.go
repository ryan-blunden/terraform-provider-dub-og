...
func (r *TagDataSourceModel) ToOperationsGetTagsRequest(ctx context.Context) (*operations.GetTagsRequest, diag.Diagnostics) {
	...
	sortBy := new(operations.GetTagsSortBy)
	if !r.SortBy.IsUnknown() && !r.SortBy.IsNull() {
		*sortBy = operations.GetTagsSortBy(r.SortBy.ValueString())
	} else {
		sortBy = nil
	}
	sortOrder := new(operations.GetTagsSortOrder)
	if !r.SortOrder.IsUnknown() && !r.SortOrder.IsNull() {
		*sortOrder = operations.GetTagsSortOrder(r.SortOrder.ValueString())
	} else {
		sortOrder = nil
	}
	search := new(string)
